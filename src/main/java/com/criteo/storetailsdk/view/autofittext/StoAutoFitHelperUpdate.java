...
/**
 * Created by MikhailPOGORELOV on 24/04/2017.
 */

public class StoAutoFitHelperUpdate {
    ...
    /**
     * Creates a new instance of {@code StoAutoFitHelper} that wraps a {@link TextView} and enables
     * automatically sizing the text to fit.
     */
    public static StoAutoFitHelperUpdate create(Button view) {
     ...
     * automatically sizing the text to fit.
     */
    public static StoAutoFitHelperUpdate create(Button view, AttributeSet attrs) {
     ...
     * automatically sizing the text to fit.
     */
    public static StoAutoFitHelperUpdate create(Button view, AttributeSet attrs, int defStyle) {
        StoAutoFitHelperUpdate helper = new StoAutoFitHelperUpdate(view);
        ...
        if (attrs != null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(
                    attrs, R.styleable.StoAutoFitButton,
                    defStyle,
                    0);
            sizeToFit = ta.getBoolean(R.styleable.StoAutoFitButton_sizeToFit, sizeToFit);
            minTextSize = ta.getDimensionPixelSize(R.styleable.StoAutoFitButton_minTextSize,
                    minTextSize);
            precision = ta.getFloat(R.styleable.StoAutoFitButton_precision, precision);
            ta.recycle();
    ...
    private static float getAutofitTextSize(CharSequence text, TextPaint paint,
        ...
        if (SPEW) StoLog.d(TAG, "low=" + low + " high=" + high + " mid=" + mid +
    ...
    private TextWatcher mTextWatcher = new StoAutoFitHelperUpdate.AutofitTextWatcher();

    private View.OnLayoutChangeListener mOnLayoutChangeListener =
            new StoAutoFitHelperUpdate.AutofitOnLayoutChangeListener();

    private StoAutoFitHelperUpdate(Button view) {
    ...
    /**
     * Adds an {@link StoAutoFitHelperUpdate.OnTextSizeChangeListener} to the list of those whose methods are called
     * whenever the {@link TextView}'s {@code textSize} changes.
     */
    public StoAutoFitHelperUpdate addOnTextSizeChangeListener(StoAutoFitHelperUpdate.OnTextSizeChangeListener listener) {
        if (mListeners == null) {
    ...
    /**
     * Removes the specified {@link StoAutoFitHelperUpdate.OnTextSizeChangeListener} from the list of those whose methods
     * are called whenever the {@link TextView}'s {@code textSize} changes.
     */
    public StoAutoFitHelperUpdate removeOnTextSizeChangeListener(StoAutoFitHelperUpdate.OnTextSizeChangeListener listener) {
        if (mListeners != null) {
     ...
     */
    public StoAutoFitHelperUpdate setPrecision(float precision) {
     ...
     */
    public StoAutoFitHelperUpdate setMinTextSize(float size) {
     ...
     * @attr ref me.grantland.R.styleable#AutofitTextView_minTextSize
     */
    public StoAutoFitHelperUpdate setMinTextSize(int unit, float size) {
        Context context = mButtonView.getContext();
        Resources r = Resources.getSystem();
     ...
     */
    public StoAutoFitHelperUpdate setMaxTextSize(float size) {
     ...
     * @attr ref android.R.styleable#TextView_textSize
     */
    public StoAutoFitHelperUpdate setMaxTextSize(int unit, float size) {
        Context context = mButtonView.getContext();
        Resources r = Resources.getSystem();
     ...
     */
    public StoAutoFitHelperUpdate setMaxLines(int lines) {
        if (mMaxLines != lines) {
            mMaxLines = lines;
     ...
     */
    public StoAutoFitHelperUpdate setEnabled(boolean enabled) {
        if (mEnabled != enabled) {
            mEnabled = enabled;

            if (enabled) {
                mButtonView.addTextChangedListener(mTextWatcher);
                mButtonView.addOnLayoutChangeListener(mOnLayoutChangeListener);

                autofit();
            } else {
                mButtonView.removeTextChangedListener(mTextWatcher);
                mButtonView.removeOnLayoutChangeListener(mOnLayoutChangeListener);

                mButtonView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSize);
            }
        }
        return this;
    }
    ...
    private void autofit() {
        float oldTextSize = mButtonView.getTextSize();
        float textSize;

        mIsAutofitting = true;
        autofit(mButtonView, mPaint, mMinTextSize, mMaxTextSize, mMaxLines, mPrecision);
        mIsAutofitting = false;

        textSize = mButtonView.getTextSize();
    ...
    private void sendTextSizeChange(float textSize, float oldTextSize) {
        ...
        for (StoAutoFitHelperUpdate.OnTextSizeChangeListener listener : mListeners) {
    ...
    /**
     * When an object of a type is attached to an {@code StoAutoFitHelper}, its methods will be called
    ...
    /**
     * Show information of attributes
     */
    public void printInformation() {
        StoLog.d(TAG, "Max lines: " + getMaxLines());
        StoLog.d(TAG, "Max Text Size: " + getTextSize());
        StoLog.d(TAG, "Max Min text Size: " + getMinTextSize());
        StoLog.d(TAG, "Max text Size: " + getMaxTextSize());
